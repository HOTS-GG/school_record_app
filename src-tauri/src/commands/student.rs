...
pub fn get_student_tags_impl(conn: &Connection, student_id: i64) -> Result<Vec<String>, String> {
    let raw: Option<String> = conn
        .query_row(
            "SELECT tags FROM Student WHERE id = ?1",
...
pub fn set_student_tags_impl(
    conn: &Connection,
    student_id: i64,
    tags: &[String],
) -> Result<(), String> {
    let json = encode_tags(tags);
    conn.execute(
        "UPDATE Student SET tags = ?1 WHERE id = ?2",
        rusqlite::params![json, student_id],
    )
    .map_err(|e| e.to_string())?;
    Ok(())
}
...
#[tauri::command]
pub fn get_student_tags(student_id: i64, state: State<DbState>) -> Result<Vec<String>, String> {
    let guard = state.0.lock().unwrap();
    let conn = guard
        .as_ref()
        .ok_or_else(|| "DB가 열려있지 않습니다.".to_string())?;
    get_student_tags_impl(conn, student_id)
}

#[tauri::command]
pub fn set_student_tags(
    student_id: i64,
    tags: Vec<String>,
    state: State<DbState>,
) -> Result<(), String> {
    ...
    let conn = guard
        .as_ref()
        .ok_or_else(|| "DB가 열려있지 않습니다.".to_string())?;
    set_student_tags_impl(conn, student_id, &tags)
}
