...
// 기본 영역 목록: (이름, byte_limit)
const DEFAULT_AREAS: &[(&str, i64)] = &[
    ("자율활동", 1500),
    ("진로활동", 1500),
    ("창체동아리", 500),
    ("자율동아리", 500),
...
pub fn get_areas_impl(conn: &Connection) -> Result<Vec<AreaItem>, String> {
    let mut stmt = conn
        .prepare(
            "SELECT a.id, a.name, a.byte_limit, a.prompt,
    ...
    for row in rows {
        let (area_id, area_name, byte_limit, prompt, act_id, act_name) =
...
pub fn create_area_impl(
    ...
    conn.execute(
        "INSERT INTO Area (name, byte_limit, prompt) VALUES (?1, ?2, ?3)",
        rusqlite::params![name, byte_limit, prompt],
    )
...
pub fn update_area_impl(
    ...
    conn.execute(
        "UPDATE Area SET name = ?1, byte_limit = ?2, prompt = ?3 WHERE id = ?4",
        rusqlite::params![name, byte_limit, prompt, id],
    )
    .map_err(|e| unique_err(&e, &format!("이미 같은 이름의 영역이 있습니다: {name}")))?;
...
pub fn create_area(
    ...
    let guard = state.0.lock().unwrap();
    let conn = guard.as_ref().ok_or_else(|| "DB가 열려있지 않습니다.".to_string())?;
    create_area_impl(conn, &name, byte_limit, prompt.as_deref())
}
...
pub fn update_area(
    ...
    let guard = state.0.lock().unwrap();
    let conn = guard.as_ref().ok_or_else(|| "DB가 열려있지 않습니다.".to_string())?;
    update_area_impl(conn, id, &name, byte_limit, prompt.as_deref())
}
...
fn seed_areas_list(conn: &rusqlite::Connection, areas: &[(&str, i64)]) -> Result<Vec<String>, String> {
    let mut added: Vec<String> = Vec::new();
    for &(name, byte_limit) in areas {
        let result = conn.execute(
            "INSERT OR IGNORE INTO Area (name, byte_limit) VALUES (?1, ?2)",
